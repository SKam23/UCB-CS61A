...
def schedule(slideshow, sum_to, max_digit):
    ...
    def schedule_helper(slideshow, sum_sofar, index):
        if sum_sofar==sum_to and len(slideshow)==0:
            return [slideshow]
        elif sum_sofar>sum_to :
            return []
        elif slideshow[index] !='&':
            return schedule_helper(slideshow[1:], sum_sofar+int(slideshow[index]), index)
        ans = []
        for x in range(0,max_digit+1) :
            modified_slideshow = schedule_helper(slideshow[1:], sum_sofar+x, index)
