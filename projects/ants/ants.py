...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if  isinstance(place.ant, ContainerAnt) and not isinstance(self, ContainerAnt) and place.ant.contained_ant == None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while curr_place.entrance != beehive:
            # if count >self.max_range or count<self.min_range:
            if rANTdom_else_none(curr_place.bees) != None:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor- amount>0:
            temp_list = self.place.bees[:]
            for beees in temp_list:
                Insect.reduce_armor(beees, amount)
        elif self.armor- amount<=0:
            temp_list = self.place.bees[:]
            for beees in temp_list:
                Insect.reduce_armor(beees, amount+self.damage)
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        temp_list = self.place.bees[:]
        for beees in temp_list:
            Insect.reduce_armor(beees,self.damage)
        # END Problem 7
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.first == False:
            ...
            while temp_place.exit != None:
                print('DEBUG:', 'called')
                if temp_place.ant != None:
                    if isinstance(temp_place.ant, ContainerAnt):
                        if  temp_place.ant.contained_ant != None and temp_place.ant.contained_ant.doubled == False :
                            temp_place.ant.contained_ant.doubled =True
                            temp_place.ant.contained_ant.damage *= 2
                        elif temp_place.ant.doubled == False:
                            temp_place.ant.damage *=2
                            temp_place.ant.doubled = True
                    else:
                        if temp_place.ant.doubled == False:
                            temp_place.ant.damage *=2
                            temp_place.ant.doubled = True
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.armor -= amount
        if self.first == True and self.armor <=0:
            bees_win()
        elif self.armor <= 0:
            self.place.remove_insect(self)
            self.death_callback()
...
def apply_status(status, bee, length):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    action_rn = bee.action
    affected_action = status(action_rn, bee)

    def do_action(gamestate):
        nonlocal length
        if length:
            affected_action(gamestate)
            length -= 1
        else:
            action_rn(gamestate)
    bee.action = do_action
