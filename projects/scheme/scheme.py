...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        # first procedure so the operator is only evaluated once
        action = scheme_eval(first, env)
        validate_procedure(action)
        if isinstance(action, MacroProcedure):
            return scheme_eval(action.apply_macro(rest, env), env)
        else:
            rest_for_argument = rest.map(lambda x: scheme_eval(x, env))
            return scheme_apply(action, rest_for_argument, env)
        # END PROBLEM 4
...
def do_or_form(expressions, env):
    ...
    #     temp_value = scheme_eval(expressions.first, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        let_opp = bindings.first
        validate_form(let_opp, 2, 2)
        operated_val = scheme_eval(bindings.first.rest.first, env)
        values = Pair(operated_val, values)
        operated_name = bindings.first.first
        names = Pair(operated_name, names)
...
def do_define_macro(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    validate_form(expressions, 2)
    macro_expres = expressions.first 
    if isinstance(macro_expres, Pair):
        func_body = expressions.rest
        func_name = macro_expres.first
        func_arg = macro_expres.rest
        validate_formals(func_arg)
        if scheme_symbolp(func_name):
            env.define(func_name, MacroProcedure(func_arg, func_body, env))
            return func_name
    raise SchemeError
