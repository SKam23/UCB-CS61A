...
def interleaved_sum(n, odd_term, even_term):
    ...
    True
    """
    def helper ( n, current, boo):
        if n == current: 
            if boo == 1:
                return odd_term(current)
            else:
                return even_term(current)
        else:
            if boo == 1:
                return odd_term(current)+helper(n, current+1, 0)
            else:
                return even_term(current)+helper(n, current+1, 1)
