...
class VendingMachine:
    ...
    def vend(self):
        if self.stock <=0:
            ...
            self.balance = 0
            self.stock -= 1
            return 'Here is your '+self.name+' and $'+str(temp)+' change.'
        else:
            self.balance -= self.price
            self.stock -= 1
    ...
    def restock(self, food_amount):
        self.stock += food_amount
...
class Coin:
    ...
    def worth(self):
        ...
        if age > 50:
            cent_worth = self.cents + age -50
        else:
            cent_worth = self.cents
        return cent_worth
class Nickel(Coin):
    cents = 5
...
def is_bst(t):
    ...
    def bst_max(t):
        if t.is_leaf():
            ...
            #max(bst_max(t.branches[1]),t.label)
    def bst_min(t):
        if t.is_leaf():
            return t.label
        else:
...
def store_digits(n):
    ...
    Final_link = Link(last_digit)
    while rest != 0:
        last_digit , rest = rest%10, rest//10
        Final_link = Link(last_digit,Final_link)
    return Final_link
...
def path_yielder(t, value):
    ...
    for branches in t.branches:
        path = path_yielder(branches,value)
        for result in path:
            yield [t.label]+result
